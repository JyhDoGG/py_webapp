    ...
    def __new__(cls, name,bases,attrs):
        ...
        for k,v in attrs.items():
            if isinstance(v,Field):
                ...
                if v.primary_key:
                    if primaryKey:
                        raise RuntimeError('Duplicate primary key for %s and %s' % (primaryKey,k))
                    primaryKey=k
                else:
                    fields.append(k)
        if not primaryKey:
...
class Model(dict,metaclass=ModelMetaClass):
    ...
    @classmethod
    async def findNumber(cls,selectField,where=None,args=None): #find number by select adn where
